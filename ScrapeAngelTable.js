...
const urlInputFile = 'urlOutputFile.csv';
const base = new Airtable({apiKey: 'key6Tigoyr6fyc4sK'}).base('appUi0FWGnwnFB3ve');
const startupView = base('Startup');


// Function reads in csv file of urls for Angel List with preconfigured key words. After the scrape() function is called to loop through the urls and scrape the table on the page.
// Param:
// Return:
function main(){
  const in_url_array = [];
  return fs.createReadStream(urlInputFile)
    .pipe(csv())
    .on('data', (data) => in_url_array.push(data[Object.keys(data)]) )
    .on('end', () => {
      scrape(in_url_array);
    })
}
...
// Return:
async function scrape(url_array) {
  for(url_i in url_array){
    // Opens the page to scrape
    await console.log(`Looking at the ${url_i} url`);
    const browser = await puppeteer.launch({headless: false});
    const page = await browser.newPage();
    // Blocks extra get requests and any css and images
    ...
    await page.goto(url_array[url_i]);
